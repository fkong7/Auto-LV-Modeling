...
class ImageLoader:
    ...
    def __init__(self, modality, data_folder, pid, fn='_test', fn_mask='_test_masks', ext='*.nii.gz'):
    ...
    def load_imagefiles(self):
        x_train_filenames = []
        y_train_filenames = []
        print(os.path.join(self.data_folder,self.modality+self.test_fn,self.pid))
        for subject_dir in sorted(glob.glob(os.path.join(self.data_folder,self.modality+self.test_fn,self.pid,self.ext))):
            x_train_filenames.append(os.path.realpath(subject_dir))
        try:
            for subject_dir in sorted(glob.glob(os.path.join(self.data_folder ,self.modality+self.test_mask_fn, self.pid, self.ext))):
